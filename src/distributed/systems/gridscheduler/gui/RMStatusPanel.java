...
public class RMStatusPanel extends StatusPanel {
	...
	private RRMCache rrmData;

	private int colWidth;
	
	public RMStatusPanel(RRMCache rrmCache) {
		this.rrmData = rrmCache;
    	
		// calculate the size needed to display the required information
		colWidth = panelWidth / 2;
		int nodeBoxSize = nodeSize + padding;
		double nodeLines = (nodeBoxSize * rrmCache.getNodeCount()) / (double)(panelWidth - padding);
...
//        System.out.println("Repainting a RMStatusPanel");
        ...
        int nrNodes = rrmData.getNodeCount();
        int nrBusyNodes = rrmData.getAmountOfBusyNodes();
        int nrDownNodes = rrmData.getAmountOfDownNodes();

        int load = (int)Math.round( (nrBusyNodes * 100) / (double)nrNodes );
        int availability = (int)Math.round( ( (nrNodes - nrDownNodes) * 100) / (double)nrNodes );

        // draw the rrmData name and load
        int x = padding;
        ...
        g.drawString("" + rrmData.getNodeCount(), x + colWidth, y);
        y += fontHeight;
        ...
        for (int i = 0; i < nrNodes; i++) {
            NodeStatus status = rrmData.getStatusOf(i);
            // determine color of the nodebox
            g.setColor(idleColor);
            if (status == NodeStatus.Busy) g.setColor(busyColor);
            if (status == NodeStatus.Down) g.setColor(downColor);
